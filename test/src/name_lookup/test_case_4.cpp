...
test_case_4()
{
	...
	*/
	...
	{
		auto found_entities = find_entities<false, true, variable>(identifier("i"), function_f->body());
		BOOST_CHECK_EQUAL(found_entities.size(), 2);
		if(found_entities.size() == 2)
		{
			BOOST_CHECK
			(
				std::find(found_entities.begin(), found_entities.end(), variable_m_i) != found_entities.end()
			);
			BOOST_CHECK
			(
				std::find(found_entities.begin(), found_entities.end(), variable_n_i) != found_entities.end()
			);
		}
	}
}
