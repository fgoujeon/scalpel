...
test_case_5()
{
	...
	*/
	...
	{
		auto found_entity = find<variable, false, false>(identifier("i"), namespace_d);
		BOOST_CHECK_EQUAL(found_entity, variable_b_i);
	}

	//look up k from namespace c, must find c::k and d::k
	{
		auto found_entities = find<variable, false, true>(identifier("k"), namespace_c);
	...
	{
		auto found_entity = find<variable, false, false>(identifier("i"), namespace_c);
		BOOST_CHECK_EQUAL(found_entity, variable_b_i);
	}

	//look up j from namespace c, must find d::j
	{
		auto found_entity = find<variable, false, false>(identifier("j"), namespace_c);
