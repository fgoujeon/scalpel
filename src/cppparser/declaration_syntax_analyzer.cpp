...
declaration_syntax_analyzer::evaluate_namespace_definition(const tree_node_t& node)
{
    ...
    else if(id == grammar::UNNAMED_NAMESPACE_DEFINITION)
    {
...
{
    assert(node.value.id() == grammar::NAMED_NAMESPACE_DEFINITION);
    ...
    const tree_node_t* body_node = find_child_node(node, grammar::NAMESPACE_BODY);
    assert(body_node);
    const tree_node_t* declaration_seq_node = find_child_node(*body_node, grammar::DECLARATION_SEQ);
