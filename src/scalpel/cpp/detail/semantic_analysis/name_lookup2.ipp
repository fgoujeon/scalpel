...
name_lookup2::find_entity
(
	std::vector<semantic_entities::declarative_region_variant> declarative_region_path,
	const std::string& name
)
{
	std::shared_ptr<EntityT> found_entity;

	//find entity from current to outermost declarative regions
	//(until global namespace)
	for(auto i = declarative_region_path.rbegin(); i != declarative_region_path.rend(); ++i)
	{
		semantic_entities::declarative_region_variant current_declarative_region = *i;
		...
		if(auto opt_namespace_ptr = boost::get<std::shared_ptr<semantic_entities::namespace_>>(&current_declarative_region))
		{
			...
			//find entity in that namespace
			found_entity =
				find_entity_in_declarative_region<EntityT>(namespace_ptr, name)
			;
		...
		else if(auto opt_class_ptr = boost::get<std::shared_ptr<semantic_entities::class_>>(&current_declarative_region))
		{
			...
			//find entity in the members of that class
			found_entity =
				find_entity_in_declarative_region<EntityT>(class_ptr, name)
			;
			if(found_entity) break;

			//find entity in the base classes of that class
			found_entity =
				find_entity_in_base_classes<EntityT>(class_ptr->base_classes(), name)
			;
			if(found_entity) break;
		}
	}
...
template<class EntityT, class DeclarativeRegionT>
std::shared_ptr<EntityT>
name_lookup2::find_entity_in_declarative_region
(
	std::shared_ptr<DeclarativeRegionT> current_declarative_region,
	const std::string& name
)
...
template<class EntityT>
std::shared_ptr<EntityT>
name_lookup2::find_entity_in_base_classes
(
	utility::shared_ptr_vector<semantic_entities::class_>::range base_classes,
	const std::string& name
)
{
	...
	for(auto i = base_classes.begin(); i != base_classes.end(); ++i)
	{
		...
		//find entity in the current declarative region (i.e. current class)
		std::shared_ptr<EntityT> current_class_found_entity = find_entity_in_declarative_region<EntityT>(current_class, name);

		//entity found?
		if(current_class_found_entity)
		{
			//return it
			found_entity = current_class_found_entity;
			break;
		}
		else
		{
			//find entity in the current declarative region's base classes
			found_entity = find_entity_in_base_classes<EntityT>(current_class->base_classes(), name);
		}
	}
